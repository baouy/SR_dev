    ...
    $(function() {
        ...
        $.validator && $.validator.config({
            ...
            rules: {
                ...
                ,bm: [/^[0-9a-zA-Z]{0,64}$/, '编码由0-64位数字、字母组成']
                ,no: [/^(?!NO$)/,'不能选择赛选'] //davdwang 加入防止错误信息
                ,character: [/^[a-zA-Z0-9\u4e00-\u9fa5]+$/g, '只能输入中文、字母和数字'] // added by Juzi
                ,bank_card:[/^([1-9]{1})(\d{14}|\d{18})$/,"请输入正确银行卡号"]// added by yushu
...
+function ($) {
    ...
    Datagrid.DEFAULTS = {
        ...
        clickRow        : null,     // Function - after clickRow method
        dblOnClick      : null,     // Function - after dblOnClick method
        lOnClick        : null,     // Function - after lOnClick method
        editdblOnclick  : false,     // true/false
        issaveallback   : null,    //截获保存是否直接访问URL
    ...
    Datagrid.prototype.TOOLS = function() {
        ...
        var tools = {
            ...
            createTrsByData: function(data, refreshFlag) {
                ...
                if (data) {
// meiwo-erp ticket-0001 json返回的数据主体由原来的data.list改为data.data。
                ...
                for (var i = 0, l = datas.length; i < l; i++) {
                    ...
                    for (var j = 0; j < modellength; j++) {
                        ...
                        if (n.calc) {
                            ...
                            if (n.calc === 'sum' || n.calc === 'avg'){
                                if(one_sum){
                                    n.calc_sum = number
                                    one_sum = false
                                }
                                else{
                                    n.calc_sum = (n.calc_sum || 0) + number
                                }
                            }
...
//-------------------------------------------------------
            ...
            appendColumns: function() {
                ...
                that.editBtnsColumn   = {name:'gridEdit', gridEdit:true, width:110, minWidth:110, label:(typeof options.showEditbtnscol === 'string' ? options.showEditbtnscol : '菜单'), align:'center', menu:false, edit:false, hide:false, quicksort:false}
            },
            // setBoxb - height
                ...
                if (that.filterDatas && !$.isEmptyObject(that.filterDatas)) {
                    $.each(that.filterDatas, function(name, v) {
                        ...
                        if (v.datas.operatorA) {
                            if (v.datas.valA instanceof Array){
                                var c  = null
                                for (var value of v.datas.valA) {
                                   c = c == null?value:c+','+value
                                }
                                var a = name+'_'
                                filterDatas[a] = c
                            }else{
                        ...
                        if (v.datas.operatorB) {
                            if (filterDatas[name]) {
                                filterDatas[name+"1"] = v.datas.valB
                                filterDatas[name +'operator1'] = v.datas.operatorB
                            } else {
                                filterDatas[name+"1"] = v.datas.valB
                                filterDatas[name +'operator1'] = v.datas.operatorB
                            }
    ...
    Datagrid.prototype.init = function() {
        ...
        that.paging = $.extend({}, {pageSize:3000, selectPageSize:'30,60,90', pageCurrent:1, total:0, showPagenum:5}, (typeof options.paging === 'object') && options.paging)
    ...
    //davidwang-更新请求
    Datagrid.prototype.refresh_editurl = function(url){
        this.options.editUrl = url
    }
    ...
    /**
     * Added By Juzi 2018-01-19
     * 直接将前台数组中的数据添加到列表中
     * @param data: {data:dataList}
     */
    Datagrid.prototype.loadArrayData = function(data) {
        var that = this, tools = that.tools;
        tools.createTrsByData(data, true);
    }
    ...
    Datagrid.prototype.initThead = function() {
        ...
        // fixed div height for th meiwo_davidwang 修改屏蔽
        if (!options.notrheight){
            setTimeout(function() {
        ...
        //头部回调
        var headsureback = options.headSureBack;
        if (typeof headsureback === 'string') {
            headsureback =  headsureback.toFunc()
            headsureback();
        }
    }
    ...
    Datagrid.prototype.initEvents = function($trs) {
        ...
        $trs.on('dblclick.bjui.datagrid.tr',function (e) {
            var $tr = $(this), index = $tr.index(), data, checked, $lockTrs = that.$lockTbody && that.$lockTbody.find('> tr')
            that.selectedRows($tr, true);
            var dbl_onclick = options.dblOnClick;
            if (typeof dbl_onclick === 'string') {
                dbl_onclick =  dbl_onclick.toFunc()
                dbl_onclick()
            }
            var $selectedTrs = that.$tbody.find('> tr.'+ that.classnames.tr_selected)
            var editdblOnclick = options.editdblOnclick;
            if (editdblOnclick) {
                if (options.inlineEditMult) {
                    that.doEditRow($selectedTrs)
                } else {
                    if (that.$lastSelect) that.doEditRow(that.$lastSelect)
                    else that.doEditRow($selectedTrs.first())
                }
            }
    ...
    Datagrid.prototype.initTfoot = function() {
        ...
        $.each(columnModel, function(i, n) {
            ...
            if (n.calc) {
                ...
                if(n.calc_sum != undefined){
                    maxnum.push(n.calc_sum)
                }else{
                    maxnum.push(0)
                }
                ...
                if (n.calc === 'avg')
                    ...
                    $th.html(calc_html.replace('#tit#', (n.calcTit || 'COUNT')).replace('#number#', (n.calc_count)))
                        ...
                        //DavidWang-修改其他运算规则
                    if (n.calclist != undefined){
                        var f = maxnum[n.calclist[0]];
                        var s = maxnum[n.calclist[1]];
                        var num = s == 0?0:(s/f);
                        var endnum = (num*100).toFixed(2);
                        $th.html(calc_html.replace('#tit#', (n.calcTit || 'SUM')).replace('#number#',endnum+'%'))
                    }else{
    ...
    Datagrid.prototype.selectedRows = function(rows, selected) {
         ...
         * 新增行点击事件
         */
        var rowclick = this.options.clickRow;
        if (typeof rowclick === 'function') {
            rowclick();
        }else if (typeof rowclick === 'string') {
            rowclick =  rowclick.toFunc()
            if (datas.length > 0){
                rowclick();
            }
        }
    ...
    Datagrid.prototype.doLock = function() {
        ...
        // fixed height
        console.log(that.$boxB[0].clientHeight);
        ...
        if (model.quickfilter) {
            ...
            if (!$filter || !$filter.length) {
                ...
                var index = $th.data('index'), model = that.columnModel[index], type = model.type || 'string', operator = model.operator || [],
                    ...
                    $select1  = $('<select data-toggle="selectpicker" data-container="true"></select>'),
                ...
                if (type === 'string' || type === 'lookup' || type === 'findgrid') {
                    ...
                    //davidwang -- 修改 > < , = !=
                    if (!operator.length) operator = ['大于', '小于']
                ...
                for (var i = (operator.length - 1); i > -1 ; i--){
                    $select1.append('<option value="'+ (operator[i]) +'">'+ (operator[i]) +'</option>')
                }
                ...
                $filterB.append($select1).append($valB)
    ...
    // inline edit davidwang-编辑状态
    Datagrid.prototype.inlineEdit = function($tr, isAdd, data) {
        if (!this.tools.beforeEdit($tr, data)) {
            return false
        }
        
        var that = this, $el_input=[] ,options = that.options, tools = that.tools, columnModel = that.columnModel, $tds = $tr.find('> td'), tds_length = $tds.length, tr_index = $tr.index(), data_index = tr_index
            ...
            var onChange = function($el, $td, val) {
                ...
                case 'select':
                    $el.change(function() {
                        ...
                        //select davidwang-wang
                        var selectchange = options.selectChange;
                        if (typeof selectchange === 'string') {
                            selectchange =  selectchange.toFunc()
                            selectchange(changeData)
                        }
                ...
                case 'lookup':
                    $td.on('customEvent.bjui.lookup', '[data-toggle="lookupbtn"]', function(e, args) {
                        ...
                        for (var key in args) {
                            ...
                            for (var n = 1 ; n < $el_input.length; n++){
                                $el = $el_input[n]
                                if (typeof $el !== 'undefined' && key == $el.prop("name")){
            ...
            if (input) {
                ...
                if (op.type === 'boolean')
                    ...
                    $input.data('context', $tr).val(String(val))
                        ...
                        //davidwang,修改change类别
                    if (op.change && typeof op.change === 'function') {
                        $input.val(String(op.change(val)))
                    }else{
    ...
    Datagrid.prototype.saveAll = function($trs) {
        ...
        if (!len) {
            var back = options.issaveallback;
        ...
        if (!len) {
            ...
            //     BJUI.ajax('doajax', opts)
            // }
// meiwo-erp ticket-0002 将编辑的提交的参数转为url提交。-----未执行改动
        ...
        if ($.fn.zTree) {
            ...
            $box.find('[data-toggle="ztree"]').each(function() {
                ...
                function _addHoverDom(treeId, treeNode) {
                    ...
                    if (!$del.length) {
                        var $del = $('<span class="tree_del hide" id="diyBtn_del_'+ treeNode.id +'" title="删除"></span>')
